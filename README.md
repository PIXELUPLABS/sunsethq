...
[GitHub Actions CI](.github/workflows/ci.yml) runs on every pull request update and push to `main`, with a manual run option. It runs tests, TypeScript, lint, and `npm run ci:build`. The check build uses the committed jobs snapshot and a public Turnstile test key, generates responsive images, and checks the production export's form fields, canonical URLs, robots directives, sitemap, assets, and secret exclusion. These checks need no repository secrets. Their output uses a test widget and must not be published.

After both checks pass on `main`, **Deploy production** builds with the real public widget key, applies D1 migrations, updates the private delivery Worker, waits for healthy recovery/Attio checks, publishes the website, and verifies production. Deployments use the GitHub `production` environment, restricted to the `main` branch. Attio and Turnstile runtime secrets remain in Cloudflare; GitHub stores only the scoped Cloudflare deployment token and the public widget key. See the [production runbook](docs/production.md#automatic-deployment).
