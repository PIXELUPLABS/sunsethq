      ...
      {DEIDENTIFICATION_TABS.map((tab, index) => {
        ...
        return (
          <div
            ...
            className={`flex flex-col justify-center gap-4 transition-opacity ease-in-out [grid-area:1/1] ${
              isActive ? "opacity-100" : "pointer-events-none opacity-0"
            }`}
          >
            <div className="flex flex-col gap-2">
              <span className="text-xs tracking-wide text-black uppercase">{tab}</span>
              <h3 className="font-serif text-2xl leading-[1.1] tracking-[-0.24px] text-black lg:text-[32px] lg:tracking-tight">
                {copy.heading}
              </h3>
            </div>
